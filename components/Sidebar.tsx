...
const Sidebar: React.FC<SidebarProps> = ({ config, activeTab, setActiveTab, updateConfig }) => {
  ...
  const wiringPatterns = [
    { id: 'row-serpentine', icon: '⇄', start: 'TL' },
    { id: 'col-serpentine', icon: '⇅', start: 'TL' },
    { id: 'row-serpentine', icon: '⇄', start: 'TR' },
    { id: 'col-serpentine', icon: '⇅', start: 'TR' },
    { id: 'row-straight', icon: '⇉', start: 'TL' },
    { id: 'col-straight', icon: '⇊', start: 'TL' },
    { id: 'row-straight', icon: '⇇', start: 'TR' },
    { id: 'col-straight', icon: '⇈', start: 'TR' },
  ];
            ...
            <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800 space-y-4">
               ...
               <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" value={config.mapWidth} onChange={(e) => updateMapWidthFromPanels(Math.max(1, parseInt(e.target.value) || 0))} className="w-full bg-zinc-950 border border-zinc-800 rounded p-2 text-sm text-white focus:border-blue-500 outline-none" />
                  </div>
                  <div>
                    <label className="block text-[10px] text-zinc-500 mb-1 uppercase font-bold">Altura (N)</label>
                    <input type="number" value={config.mapHeight} onChange={(e) => updateMapHeightFromPanels(Math.max(1, parseInt(e.target.value) || 0))} className="w-full bg-zinc-950 border border-zinc-800 rounded p-2 text-sm text-white focus:border-blue-500 outline-none" />
                  </div>
               </div>
            </div>
            ...
            <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800 space-y-4">
               ...
               <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" step="0.1" value={config.targetWidthM} onChange={(e) => updateMapWidthFromMeters(parseFloat(e.target.value) || 0)} className="w-full bg-zinc-950 border border-zinc-800 rounded p-2 text-sm text-white focus:border-blue-500 outline-none" />
                  </div>
                  <div>
                    <label className="block text-[10px] text-zinc-500 mb-1 uppercase font-bold">Altura (M)</label>
                    <input type="number" step="0.1" value={config.targetHeightM} onChange={(e) => updateMapHeightFromMeters(parseFloat(e.target.value) || 0)} className="w-full bg-zinc-950 border border-zinc-800 rounded p-2 text-sm text-white focus:border-blue-500 outline-none" />
                  </div>
               </div>
            </div>
            ...
            <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800 space-y-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                {wiringPatterns.map((p, i) => (
                  <button key={i} onClick={() => updateConfig({ wiringPattern: p.id as any, wiringStartCorner: p.start as any })} className={`aspect-square flex items-center justify-center rounded border transition-all ${config.wiringPattern === p.id && config.wiringStartCorner === p.start ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-zinc-950 border-zinc-800 text-zinc-600 hover:text-zinc-300'}`}>
